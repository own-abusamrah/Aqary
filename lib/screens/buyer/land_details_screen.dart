...
class _LandDetailsScreenState extends State<LandDetailsScreen> {
  ...
  void _watchContactRequest() {
    if (_uid == null) { setState(() => _loadingContact = false); return; }
    ContactRequestService.instance.watchRequestStatus(
      buyerId: _uid!, listingId: listing.id,
    ).listen((request) async {
      if (!mounted) return;
      setState(() { _contactRequest = request; _loadingContact = false; });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
          ...
          flexibleSpace: FlexibleSpaceBar(
            background: listing.photoUrls.isNotEmpty
                ? PageView.builder(
                    ...
                    itemBuilder: (_, i) => Image.network(listing.photoUrls[i], fit: BoxFit.cover))
                : Container(color: AppTheme.background,
                    child: const Icon(Icons.landscape, size: 72, color: AppTheme.textMuted))),
          actions: [
            IconButton(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            if (listing.photoUrls.length > 1) ...[
              Row(mainAxisAlignment: MainAxisAlignment.center,
                children: List.generate(listing.photoUrls.length, (i) =>
                  AnimatedContainer(duration: const Duration(milliseconds: 200),
                    margin: const EdgeInsets.symmetric(horizontal: 3),
                    width: _currentPhotoIndex == i ? 20 : 8, height: 8,
                    decoration: BoxDecoration(
                      color: _currentPhotoIndex == i ? AppTheme.primary : const Color(0xFFE5E7EB),
            ...
            Row(children: [
              _SpecBadge(Icons.straighten, '${listing.size.toStringAsFixed(0)} m²'),
            ...
            Row(children: [
              ...
              Text(listing.area, style: const TextStyle(fontSize: 14, color: AppTheme.textMuted)),
            ]),
            const SizedBox(height: 20),
            const Text('Description', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: AppTheme.textDark)),
            const SizedBox(height: 8),
            Text(listing.description, style: const TextStyle(fontSize: 14, color: AppTheme.textMuted, height: 1.6)),
                          ...
                          Row(
                            children: [
                              ...
                              IconButton(
                                ...
                                onPressed: () => _callNumber(_approvedContactInfo!),
                              ),
                              ...
                              IconButton(
                                ...
                                onPressed: () => _openWhatsApp(
                                  _approvedContactInfo!,
                                  message: "Hi, I'm interested in your land listing on Aqary.",
                                ),
                              ),
                            ],
                          ),
            ...
            const Text('Location', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: AppTheme.textDark)),
            const SizedBox(height: 8),
            ...
            OutlinedButton.icon(
              onPressed: () => Navigator.of(context).push(MaterialPageRoute(
                builder: (_) => ProvidersListScreen(
                    nearLat: listing.latitude, nearLng: listing.longitude))),
              icon: const Icon(Icons.engineering_rounded, color: AppTheme.primary),
      ...
      bottomNavigationBar: _loadingContact
          ? null
          : _ContactBar(request: _contactRequest, existingSellerRequest: _existingSellerRequest, onRequest: _requestContact),
    );
  }
}
...
class _SpecBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Row(mainAxisSize: MainAxisSize.min, children: [
      Icon(icon, size: 14, color: AppTheme.primary), const SizedBox(width: 4),
      Text(label, style: const TextStyle(fontSize: 13, color: AppTheme.primary, fontWeight: FontWeight.w500))]));
}
...
class _ContactBar extends StatelessWidget {
  ...
  const _ContactBar({required this.request, required this.existingSellerRequest, required this.onRequest});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      :switch (request?.status) {
        null => ElevatedButton.icon(onPressed: onRequest,
            icon: const Icon(Icons.contact_phone_outlined), label: const Text('Request Contact')),
        'pending' => Container(height: 52, alignment: Alignment.center,
            decoration: BoxDecoration(color: AppTheme.accent.withValues(alpha: 0.12), borderRadius: BorderRadius.circular(12)),
            child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              Icon(Icons.hourglass_top, color: AppTheme.accent), SizedBox(width: 8),
              Text('Request Pending', style: TextStyle(color: AppTheme.accent, fontWeight: FontWeight.w600))])),
        'approved' => Container(height: 52, alignment: Alignment.center,
            decoration: BoxDecoration(color: AppTheme.success.withValues(alpha: 0.12), borderRadius: BorderRadius.circular(12)),
            child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              Icon(Icons.check_circle_outline, color: AppTheme.success), SizedBox(width: 8),
              Text('Contact info shown above', style: TextStyle(color: AppTheme.success))])),
        _ => Container(height: 52, alignment: Alignment.center,
            decoration: BoxDecoration(color: AppTheme.error.withValues(alpha: 0.10), borderRadius: BorderRadius.circular(12)),
            child: const Text('Request Declined', style: TextStyle(color: AppTheme.error))),
      },
    );
  }
}
